...
void rest_post(float temp, float hum, char time[100], uint32_t batteryLevel)
{
	const char *post_data;
    char temp_str[32];
    char hum_str[32];
    char batteryLevel_str[32];
	...
	cJSON_AddStringToObject(root, "Battery Level", batteryLevel_str);
